...
- **The booths the guide never carded, saveable anyway.** The guide writes
  ...
  stand only in the entertainment halls — mostly indie studios sharing one of
  hall 10's collective stands. Tap such a stand on the map and the sheet lists
  whoever is filed on it, each saveable as a stop of your own: hall 10.2's
  Indie Arena Booth is one footprint with 171 studios on it, so the list has a
  filter box and puts the ones you have marked at the top. Every row of the
  full directory carries the same save button, which is the other way you find
  one name among 1,785. A booth saved this way is a stop like any other — it
  takes a day, lights its stand, gets numbered on the route and exports to
  your calendar — and it never pretends to be more than it is: no queue call,
  no lineup, no crowd forecast, and the map still tells you at a glance which
  booths the guide actually covers and which it does not
...
Once the show is nearly over the guide asks the only question it has: was this
worth doing, and should it come back for 2027. A card above every view, and a
link in the footer, both pointing at a short hosted form. `docs/FEEDBACK-FORM.md`
holds the questions in both languages and the setup.

It is a hosted form rather than an endpoint in `worker/`, for a reason that is
about timing rather than taste. That API's D1 database is deleted after the show
— `docs/DEPLOYING.md` removes the bindings and drops both databases once the last
deferred queue outcome has landed, because the privacy promise is deletion — while
feedback arrives on the opposite schedule: a little on the last day, most of it in
the week after. A table there would be torn down in the middle of its own
collection window. Free text would also be the first thing the guide ever stored
...
An empty `url` hides every surface. `urlDe` is optional and falls back to `url`.
`from` is an ISO date read in Cologne's timezone — the card stays hidden before
it, and there is no end date, because after the show is when the answers come in.
The card asks once: answering it or dismissing it writes `gc2026.feedback.v1` and
it never returns on that device. The footer link has no memory, which is what
