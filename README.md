...
| `data/hallplan/entrances.json` | Where each hall opens onto the Boulevard or into its neighbour. Hand-written, and the one file in that directory the tool never touches: the official data files blocks and stands, never a wall or a doorway |
...
data and are not this project's to relicense (`entrances.json` beside them is
