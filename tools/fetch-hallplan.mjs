/* Snapshot booth geometry for the hall map from Koelnmesse's exhibitor
   ...
   The last one comes from the hall-plan page rather than that endpoint:
   the page carries a `hallen` array of campus outlines — every hall, the
   Boulevard, the passages between them and the entrances — which is what
   the map's overview is drawn from. See campusSnapshot() for what that
   array is and is not good for.
   ...
   Run it from anywhere (no dependencies):
     ...
     node tools/fetch-hallplan.mjs --campus    # re-snapshot the campus layout only
...
/* ---- the campus overview ----
   ...
   The hall-plan page draws the whole site before you pick a hall, from a
   `hallen` array of outlines: every hall, the Boulevard, the Piazza, each
   passage and Durchgang between two halls, the outdoor areas and the
   Confex. That array is the only published description of how the halls
   sit relative to each other, and it is what data/hallplan/campus.json
   is a snapshot of.

   What it is not is a plan. Its rings are on a 5-unit grid and each hall
   is fitted to its slot on the artwork rather than drawn to scale — hall
   6 comes out at ~1.5 m per unit across and ~1.4 m down, hall 4 at ~1.6
   and ~2.1 — so two neighbouring halls are at different scales and no
   single scale converts the file to metres. It is a diagram: which hall
   is where, what joins it to what. The map says so where it draws it.

   The note below travels with the file, like the one in outline.json: a
   reader who meets campus.json on its own has to learn what it is right
   about before they trust a number in it. */
const CAMPUS_NOTE =
  "Generated by tools/fetch-hallplan.mjs from the `hallen` array on the " +
  "hall-plan page (SOURCE below) — the outline Koelnmesse draws the whole " +
  "site from before you pick a hall. A DIAGRAM, NOT A PLAN: the rings are on " +
  "a 5-unit grid and every hall is fitted to its slot on the official artwork " +
  "rather than drawn to scale, so neighbouring halls come out at different " +
  "scales (hall 6 at about 1.5 m per unit across, hall 4 at 1.6, and 1.4 " +
  "against 2.1 down) and no single scale converts this file to metres. What " +
  "it is right about is the arrangement: which hall is where, which wall " +
  "faces the Boulevard, and what joins what — which is the whole job of an " +
  "overview. A hall's own geometry is in its hall-<id>.json, in true metres. " +
  "FRAME: x runs west to east and y north to south, the same way round as a " +
  "hall file, with 0,0 at the north-west corner of everything drawn; `size` " +
  "is the extent. FEATURES: kind is hall, concourse (the Boulevard and the " +
  "Piazza), passage (every Durchgang and Passage between two halls, and the " +
  "entrance halls), outdoor (the open-air areas) or building (the Confex and " +
  "the Congress-Centrum wings). id is Koelnmesse's own code, which for a hall " +
  "is its number. `levels` names the hall levels this guide draws, so the " +
  "overview knows which plates open into a map and which are there for " +
...
/* Codes are bare ("B1", "D67", "EINO") and the page labels none of them.
   These are the ones worth naming on a diagram, as i18n keys the client
   resolves from map.place.*; everything else draws as what its first
   letter says it is (see campusKind) and carries no name. Ours, like
   AREAS.access — there is no label at the source to snapshot. */
const CAMPUS_PLACES = {
  B: "boulevard",
  B1: "boulevard",
  B3: "piazza",
...
/* The artwork is laid over the same map as the outlines, so its frame is
   the outlines' frame shifted: the polygons are placed at lat 1036 − y and
   the artwork spans lat 0…902 with a viewBox 903.4 deep, which puts its
   own y this far below theirs. (The 902/903.4 stretch is 0.15 %, a sixth
   of a grid unit across the whole site — below the precision of anything
   in this file.) */
const ARTWORK_DY = 134;
...
/* The area colours in AREAS, checked against their source.
   ...
   is not a reason to refuse to refresh it. The same goes for the campus
   layout read out of the same page below — which is why the page is
   fetched once, by the caller, and handed to both. */
async function fetchPlanPage() {
  try {
    ...
    console.log(`hall-plan page: could not read it (${err.message})`);
    return null;
  }
}

function checkAreaColours(html) {
  if (!html) {
    console.log("colours: no page to check against — keeping the committed area colours");
    return;
  }
...
/* ---- the campus overview: data/hallplan/campus.json ---- */

/* What a bare campus code is. The page's own helpers test for a letter
   anywhere in the code, which overlaps on half of them ("DP2B3" answers
   yes to Durchgang, Passage and Boulevard); the first letter is what
   actually names the thing, so that is what this reads.

   D and P are one kind here. Koelnmesse distinguishes a Durchgang (a
   doorway from one hall straight into the next) from a Passage (a walked
   corridor between two), which matters when you are letting a stand out
   and not at all when you are finding your way: both are the way through. */
function campusKind(code) {
  if (/^\d/.test(code)) return "hall";
  return { B: "concourse", D: "passage", P: "passage", F: "outdoor", C: "building", M: "building", E: "passage" }[
    code[0]
  ] || "building";
}

/* The `hallen` array as JSON. It is hand-maintained JavaScript — single
   quotes, bare keys, commented-out entries, a trailing comma — so it is
   converted rather than eval'd: this file never runs code it fetched. */
function parseHallen(html) {
  const block = html.match(/var\s+hallen\s*=\s*\[([\s\S]*?)\n\];/);
  if (!block) return null;
  const src = block[1]
    .replace(/\/\*[\s\S]*?\*\//g, "")                  /* retired entries */
    .replace(/\/\/[^\n]*/g, "")                        /* the "// Durchgang 6 zu 7" notes */
    .replace(/'([^'\\]*)'/g, '"$1"')                   /* 'B1' → "B1" */
...
/* Where each entrance stands, from the dots on the page's site artwork.
   Each entrance is a <g> of its own, and the first path in it is the dot;
   the rest of the group is the leader line and the name set letter by
   letter, which we neither need nor take. */
function parseEntrances(html) {
  const out = [];
  for (const [id, key] of Object.entries(CAMPUS_ENTRANCES)) {
    const at = html.indexOf(`<g id="${id}">`);
    if (at === -1) continue;
    const dot = html.slice(at, at + 4000).match(/\bd="m\s*(-?[\d.]+),(-?[\d.]+)/);
    if (dot) out.push({ id: key, at: [+dot[1], +dot[2] + ARTWORK_DY] });
  }
  return out;
}

/* The whole site as one ring list, in the frame the outlines are filed in:
   x east, y south, the same way round as a hall's own frame — so north is
   up on the overview exactly as it is on a hall.

   Seven of the entries are placed by a different rule at the source: the
   ones flagged `pure` are drawn from a second origin, with their dx/dy as
   an offset rather than as the mirror signs everything else carries. They
   are the doorways around hall 1 and the Confex, the Boulevard's second
   way into hall 10, and the run alongside the south entrance; left
   unshifted they land in the wrong quarter of the site, so the shift is
   applied here.

   Normalised to start at 0,0 like a hall file, so the client draws a
   viewBox from `size` and nothing needs to know where Koelnmesse's origin
   was. Rounded to the unit: the grid these live on is 5. */
function campusSnapshot(html) {
  if (!html) return null;
  const raw = parseHallen(html);
  if (!raw) {
    console.log("campus: could not read the hallen array on the hall-plan page — keeping the committed layout");
    return null;
  }

  const levelsOf = (code) =>
    Object.entries(HALLS)
      .filter(([, [halle]]) => halle === code)
      .map(([id]) => id);

  const features = [];
  const seen = new Set();
  for (const entry of raw) {
    const code = String(entry.halle);
    const ring = (entry.coord || []).map(([x, y]) =>
      entry.pure ? [x + 25 + (entry.dx || 0), y + 225 - (entry.dy || 0)] : [x, y]
    );
    if (ring.length < 3) continue;
    /* The Confex is filed twice, once under each of two codes, on the
       same ring. One shape, one feature. */
    const key = ring.map((p) => p.join()).join(" ");
    if (seen.has(key)) continue;
    seen.add(key);

    const kind = campusKind(code);
    const levels = kind === "hall" ? levelsOf(code) : [];
    features.push({
      kind,
      id: code,
      ...(levels.length ? { levels, area: HALLS[levels[0]][2] } : {}),
      ...(CAMPUS_PLACES[code] ? { place: CAMPUS_PLACES[code] } : {}),
      poly: ring,
    });
  }

  /* An array we can parse but that holds no hall at all means the page has
     changed shape under us. A warning and the committed layout, not a
     throw: unlike the colour check, nothing about the booths depends on
     this, and a refresh of them must not fail over the overview. */
  if (!features.some((f) => f.kind === "hall")) {
    console.log("campus: the hallen array holds no numbered hall — keeping the committed layout");
    return null;
  }
  const entrances = parseEntrances(html);

  /* One box round everything drawn, entrances included: an entrance dot
     outside it would be clipped by the client's viewBox. */
  const pts = [...features.flatMap((f) => f.poly), ...entrances.map((e) => e.at)];
  const x0 = Math.min(...pts.map((p) => p[0]));
  const y0 = Math.min(...pts.map((p) => p[1]));
  const shift = ([x, y]) => [Math.round(x - x0), Math.round(y - y0)];
  for (const f of features) f.poly = f.poly.map(shift);
  for (const e of entrances) e.at = shift(e.at);
  const all = [...features.flatMap((f) => f.poly), ...entrances.map((e) => e.at)];

  return {
    fetched: new Date().toISOString().slice(0, 10),
    source: PLAN_PAGE,
    size: [Math.max(...all.map((p) => p[0])), Math.max(...all.map((p) => p[1]))],
    features,
    entrances,
  };
}

/* One feature per line, like the hall files: a Koelnmesse edit to a single
   passage shows up as a single changed line. */
function serialiseCampus(campus) {
  const rows = (list) => list.map((f) => "  " + JSON.stringify(f)).join(",\n");
  return `{
"note": ${JSON.stringify(CAMPUS_NOTE)},
"fetched": ${JSON.stringify(campus.fetched)},
"source": ${JSON.stringify(campus.source)},
"size": ${JSON.stringify(campus.size)},
"features": [
${rows(campus.features)}
],
"entrances": [
${rows(campus.entrances)}
]
}
`;
}
...
/* The campus layout on its own. It comes off one page rather than the
   booth endpoint, it changes when Koelnmesse rebuilds the site rather than
   when an exhibitor moves stand, and re-reading it should not mean asking
   them for twelve halls of booths again. */
if (process.argv.includes("--campus")) {
  const campus = campusSnapshot(await fetchPlanPage());
  if (!campus) {
    console.log("campus: nothing written");
    process.exit(1);
  }
  mkdirSync(OUT, { recursive: true });
  writeFileSync(join(OUT, "campus.json"), serialiseCampus(campus));
  console.log(
    `wrote data/hallplan/campus.json (${campus.features.length} features, ` +
      `${campus.entrances.length} entrances, ${campus.size[0]}×${campus.size[1]})`
  );
  process.exit(0);
}
...
const planPage = await fetchPlanPage();
checkAreaColours(planPage);
/* Optional, like the colour check and for the same reason: the overview is
   worth a lot less than the halls, and a markup change at the source must
   not cost us a booth refresh. A failed read keeps the committed layout. */
const campus = campusSnapshot(planPage);

mkdirSync(OUT, { recursive: true });
for (const hall of halls) writeFileSync(join(OUT, `hall-${hall.hall}.json`), serialise(hall));
if (campus) writeFileSync(join(OUT, "campus.json"), serialiseCampus(campus));
writeFileSync(
  join(OUT, "index.json"),
  JSON.stringify(
    {
...
console.log(
  `wrote data/hallplan/ (${halls.length} halls + index${campus ? ` + campus, ${campus.features.length} features` : ""})`
);
joinReport(halls);
